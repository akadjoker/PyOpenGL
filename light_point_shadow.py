...
while core.run():
    ...
    for face in range(6):
        buffer.render(face)        
        depthShader.set_matrix("lightSpaceMatrix",  shadowTransforms[face])
    ...
    #Render.set_texture_cube(buffer.depth)
    ...
    if showQuad:
        ...
        #Render.set_texture_cube_face(buffer.id, 0)
        Render.set_texture(buffer.id, 0)
        
        quadrender.render(0.0,0.0,1.0,1.0)


        #Render.set_texture_cube_face(0, 0)
